...
rotor_area = np.pi * rotor_radius**2  # m²
timezone_offset = -5  # Mount Storm, WV = Eastern Time

# Wind Farm Parameters
turbine_rating_mw = 2.5  # MW
turbine_count = 16  # Total turbines in the farm
...
# === Compute wind speed & single turbine power
wind_speed = np.sqrt(u_flat**2 + v_flat**2)
turbine_power_mw = 0.5 * air_density * rotor_area * wind_speed**3 / 1_000_000  # Single turbine power (MW)

# === Scale to full farm
wind_farm_power_mw = turbine_power_mw * turbine_count
...
df = pd.DataFrame({
    ...
    'wind_power_mw': wind_farm_power_mw
})

# === Drop rows where wind_power_mw is NaN
df = df.dropna(subset=['wind_power_mw'])
