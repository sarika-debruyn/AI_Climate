# Bayesian Optimization for Solar NGBoost
...
from main import load_solar_data, prepare_features, generate_2024_features, add_zenith_angle, FEATURE_COLUMNS
...
def objective(trial):
    ...
    ghi_pred = model.predict(X_forecast_scaled)

    # Evaluate performance (use MAE on GHI, W/m²)
    mae = mean_absolute_error(ghi_pred, y_train[-len(ghi_pred):])
...
print(f" Best MAE: {study.best_value:.4f} W/m²")
